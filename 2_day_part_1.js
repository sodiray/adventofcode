...
const passwordIsValid = ({ frequency, count }) => {
  const [min, max] = frequency
  return  min <= count && count <= max
}

const applyCharCount = (context) => {
  const { char, password } = context
  const count = (password.match(new RegExp(char, "gi")) || []).length
  return {
    ...context,
    count
  }
...
const parsePasswordInput = (inputItem) => {
  ...
  return {
    frequency: [parseInt(match[1]), parseInt(match[2])],
    char: match[3],
    password: match[4]
  }
}
