...
const passwordIsValid = ({ values, char }) => {
  const [a, b] = values
  if (a === char && b !== char) return true
  if (b === char && a !== char) return true
  return false
}

const applyLocationValues = (context) => {
  const { locations, password } = context
  const [ai, bi] = locations
  return {
    ...context,
    values: [
      password[ai - 1],
...
const parsePasswordInput = (inputItem) => {
  const regex = /(\d+)\-(\d+)\s(\w)\:\s(\w+)/
  const match = regex.exec(inputItem)
  return {
    locations: [parseInt(match[1]), parseInt(match[2])],
    char: match[3],
    password: match[4]
  }
}

const [pass, fail] = input.reduce((acc, item) => {
  const [p, f] = acc
  const isValid = (
    passwordIsValid(
      applyLocationValues(
        parsePasswordInput(item))))
  console.log(`${isValid ? 'PASS' : 'FAIL'} => ${item}`)
  if (isValid) return [p+1, f]
  else return [p, f+1]
}, [0, 0])
